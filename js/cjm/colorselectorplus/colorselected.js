...
function colorSelected(id, value, product_image_src, front_label) {
	...
	if (dropdownEl.disabled) { 
		//alert("Bitte wählen sie erst eine Farbe aus.");

		// Flemming | M4M | Schrift einblenden
		if(document.getElementById("SizeFail123456")){}else{
			var NewDiv		=	document.createElement('div');
			NewDiv.setAttribute('id', 'SizeFail123456');
			NewDiv.innerHTML	= "Bitte erst eine Farbe w&auml;hlen!";
			dropdownEl.parentElement.parentElement.appendChild(NewDiv);
		}
		SizeFailElement = true;
		//var containerxy = Target.html();
		//containerxy = "<div class='alert'>dies ist ein Fehler</div>" + containerxy;
		//Target.html("DIES IST EIN TESTWERT");
		return; 
	}
	...
	if (nextAttribute !== 'none' && value) {
		...
		$$('#ul-attribute' + nextAttribute + ' .swatch').each(function(item) {
			// FLemming | M4M | Wenn Element vorhanden dieses wieder ausblenden!!
			if(document.getElementById("SizeFail123456")){
				document.getElementById("SizeFail123456").remove();
			}
			theoptions.contains(item.id.substr(6)) ? $(item.id).removeClassName('disabledSwatch') : $(item.id).addClassName('disabledSwatch');
		});
	} 
