...
*******************************************************************************************/
    ...
    try {		 
		// Get Soap Connection
			...
			$session = $client->login('dmconnector', 'dmc0511');	
	...
	$query	= "SELECT sku,image FROM `catalog_product_flat_1` where (`image` is NOT null AND `image` <>'')  ORDER BY `sku` ASC";	
	...
	while($row = mysql_fetch_array($result)){
		...
		{
			if (!copy('../media/catalog/product'.$Artikel_Bilddatei, './export_bilder/'.$Artikel_Artikelnr.'.jpg')) {
				echo "copy ".'../media/catalog/product'.$Artikel_Bilddatei." zu ".'./export_bilder/'.$Artikel_Artikelnr." schlug fehl...\n";
			}
		}
	} // end while
